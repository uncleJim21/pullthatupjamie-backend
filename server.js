...
app.post('/api/pull', serviceHmac({ optional: true }), createEntitlementMiddleware(ENTITLEMENT_TYPES.PULL), (req, res, next) => {
  ...
  /* #swagger.parameters['amountSats'] = {
    ...
    minimum: 10,
    maximum: 500000,
    description: 'Optional. Specifies the sats amount for the returned Lightning invoice when calling without an L402 credential (to receive a 402 challenge). When omitted, the invoice is auto-sized to cover approximately one call at the current BTC/USD rate (cost + ~15% buffer). Use this to prefund multiple calls or to purchase bulk credits. Silently ignored when an Authorization: L402 header is already present — the existing balance is debited instead.'
  } */
  /* #swagger.parameters['body'] = {
    in: 'body',
    required: true,
...
app.listen(PORT, async () => {
  ...
  try {
    ...
    try {
      ...
      console.warn('[startup-banner] failed to render default-model banner:', bannerErr.message);
    }
  } catch (error) {
