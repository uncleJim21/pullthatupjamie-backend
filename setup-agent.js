...
- **Answer in the user's language.** Write the entire answer — prose, headers, and quotes — in the same language the user's question is written in (English → English, Spanish → Spanish, German → German). This is the language of the QUESTION, not the language of the clips. When a cited clip is in a different language, translate its quoted excerpt into the user's language inside the blockquote — do not keep the original-language text or add "(translated)" notes. The client tells the reader each clip's source language. E.g. for an English question citing a Spanish clip, *"Ahora mismo estamos discutiendo Bitcoin"* becomes *"Right now we are discussing Bitcoin"*; for a Spanish question citing an English clip, *"Bitcoin is a hedge against inflation"* becomes *"Bitcoin es una cobertura contra la inflación"*.
...
4. **Zero invention.** Only state facts, dates, episode titles, descriptions, and summaries that are explicitly present verbatim in the tool results above. Do NOT write episode descriptions, \"key thoughts\", or \"additional insights\" sections that go beyond what the search results directly contain. If a piece of information is not in the tool results, it does not go in the answer.
   - **Language exception (translation only).** This zero-invention rule governs facts and clip IDs — it does NOT forbid translating a quote's display text into the user's language (see the language rule below). Faithfully translating what the speaker actually said is not invention. Keep the \`{{clip:…}}\` token exact and unchanged; render the words inside the blockquote in the answer's language. This carve-out applies ONLY to language — everything else stays verbatim. Examples: Spanish clip in an English answer → *"Bitcoin es el dinero más perfecto"* becomes *"Bitcoin is the most perfect money"*; *"El Salvador ha adoptado a Bitcoin como moneda de curso legal"* becomes *"El Salvador has adopted Bitcoin as legal tender"*; a German clip → *"Bitcoin ist digitales Gold"* becomes *"Bitcoin is digital gold"*.
...
9. **Answer in the user's language.** Write your entire answer — prose, headers, and quotes — in the same language the user wrote their question in (English question → English answer, Spanish → Spanish, German → German). This is the language of the QUESTION, not the language of the clips. When a clip you cite is in a different language, translate its quoted excerpt into the user's language inside the \`> *"..."*\` blockquote — do not keep the original-language text. The client shows the reader which source language each clip came from, so translate freely and do not add "(translated)" notes or the original wording yourself. Translate quotes like these:
   - English question, Spanish clip → *"Ahora mismo estamos discutiendo Bitcoin"* becomes *"Right now we are discussing Bitcoin"*
   - English question, German clip → *"Bitcoin ist digitales Gold"* becomes *"Bitcoin is digital gold"*
   - Spanish question, English clip → *"Bitcoin is a hedge against inflation"* becomes *"Bitcoin es una cobertura contra la inflación"*
   - German question, Spanish clip → *"El Bitcoin es dinero para la gente"* becomes *"Bitcoin ist Geld für die Menschen"*`;
...
8. **Answer in the user's language.** Write the entire answer — prose and quotes — in the same language the user's question is written in (English → English, Spanish → Spanish, German → German), NOT the language of the clips. Translate any non-matching quoted excerpt into the user's language inside the blockquote; do not keep the original-language text or add "(translated)" notes. The client tells the reader each clip's source language. E.g. English question, Spanish clip: *"Ahora mismo estamos discutiendo Bitcoin"* becomes *"Right now we are discussing Bitcoin"*.
